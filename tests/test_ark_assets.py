...
class ArkAssetTests(unittest.TestCase):
    ...
    def test_external_review_gate_contract_assets_exist(self) -> None:
        ...
        test_skill = (ROOT / "skills/ark-test/SKILL.md").read_text()
        sync_skill = (ROOT / "skills/ark-sync/SKILL.md").read_text()
        next_skill = (ROOT / "skills/ark-next/SKILL.md").read_text()
        handoff_skill = (ROOT / "skills/ark-handoff/SKILL.md").read_text()
        validate_skill = (ROOT / "skills/ark-validate/SKILL.md").read_text()
        ...
        self.assertIn("外部 findings 修复模式", debug_skill)
        self.assertIn("Finding 闭合状态", debug_skill)
        self.assertIn("复检关注点", debug_skill)
        self.assertIn("给 `/ark:ark-test` 的 finding 级测试目标", debug_skill)
        self.assertIn("给 `/ark:ark-review-gate recheck` 的闭合摘要", debug_skill)
        self.assertIn("Finding 覆盖映射", test_skill)
        self.assertIn("覆盖测试", test_skill)
        self.assertIn("未覆盖风险", test_skill)
        self.assertIn("External Review Gate 状态漂移", sync_skill)
        self.assertIn("findings-imported", sync_skill)
        self.assertIn("不得直接写 `validation.md`", sync_skill)
        self.assertIn("package-prepared", next_skill)
        self.assertIn("Finding 闭合状态", next_skill)
        self.assertIn("/ark:ark-debug", next_skill)
        self.assertIn("Finding 闭合状态", handoff_skill)
        self.assertIn("/ark:ark-debug", handoff_skill)
        ...
        self.assertIn("Finding 闭合状态", handoff_template)
