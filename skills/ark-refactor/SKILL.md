---
name: ark-refactor
description: |
  在保持外部行为稳定的前提下改善代码结构。
  触发时机：代码存在明显可维护性问题、重复与复杂度已造成实际负担、希望优化结构而不是新增功能时。
  关键词：重构、refactor、改善结构、去重、模块化、可维护性、清理代码。
version: "1.0"
---

# /ark-refactor

## 目标
在保持外部行为稳定的前提下改善代码结构。

## 前置建议
- 如果 Artifact 文档可能已过期，建议先执行 `/ark:ark-sync` 同步状态

## 适用场景
- 代码存在明显可维护性或清晰度问题
- 重复与复杂度已造成实际负担
- 希望优化结构而不是新增功能

## 不适用场景
- 当前任务本质上是加新功能
- 不变行为边界不清楚
- 当前核心问题是 bug，而不是结构问题

## 输入
- 当前代码、预期不变行为、相关测试、相关 design 或 plan Artifact
- 外部检查结果（可选）：如 SonarLint、mypy、ruff 等工具输出

## 输出
- 改善后的结构、明确列出的不变行为、建议或补充的回归覆盖、剩余风险说明

## 相关 Artifact
- 可引用 `docs/ark/spec.md`、`docs/ark/design.md`、`docs/ark/plan.md`
- 若重构改变设计现实：建议 `/ark:ark-design`，不得直接回写 `design.md`
- 若重构发现外部行为或能力边界实际变化：停止扩大重构范围，建议 `/ark:ark-spec` 或 `/ark:ark-design`
- 若重构改变专题详细设计、契约组织、集成方式或数据源元信息：建议 `/ark:ark-solution`
- 应为 `docs/ark/validation.md` 提供输入

## 工作流
1. 先定义哪些行为不能变。
2. 识别需要优化的结构性问题。
3. 采用渐进式重构。
4. 避免把新功能混入同一次重构。
5. 在需要时加强回归安全网。
6. 检查是否出现 spec/design/extension 漂移：重构应保持外部行为稳定；若设计现实变化则建议 `/ark:ark-design`，若扩展文档现实变化则建议 `/ark:ark-solution`，若外部行为边界变化则停止并建议重新确认。

## 验证要求
- 不变行为必须显式化
- 应在可行范围内检查行为稳定性
- 若存在混合 scope，应主动指出
- 若模块边界、依赖方向、接口组织、资源生命周期发生变化，应建议 `/ark:ark-design`
- 若外部行为、能力范围、验收标准发生变化，不得继续把它当作纯重构；应建议 `/ark:ark-spec` 或 `/ark:ark-design`
- 若专题详细设计、接口契约、集成方式或数据源元信息发生变化，应建议 `/ark:ark-solution`

## 停止条件
- 结构目标得到改善，且行为变化风险处于可接受范围

## 固定输出格式

### 1. 重构范围
- 修改了哪些模块 / 文件
- 本次明确不处理的范围

### 2. 不变行为约束
- 外部接口不变项
- 用户可观察行为不变项
- 数据格式 / 错误行为 / 兼容性不变项

### 3. 结构改进
- 已完成的结构变化
- 解决了哪些重复、复杂度或职责问题

### 4. 回归覆盖
- 已有测试是否覆盖关键行为
- 新增 / 更新的测试
- 未覆盖内容及原因

### 5. 剩余风险
- 行为变化风险
- 需要后续验证的点

### 5.5 Artifact 更新建议
- `design.md`：建议 `/ark:ark-design` / 无需更新
- `spec.md`：建议 `/ark:ark-spec` / 无需更新
- 扩展文档：建议 `/ark:ark-solution` / 无需更新

### 6. 建议下一步
- 通常：`/ark:ark-test` → `/ark:ark-validate`

## 备注
明确说明本次重构的非功能目标，例如：清晰度、模块化、可测试性、去重。
