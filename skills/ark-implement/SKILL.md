...
## 输出
- 已完成的代码修改
- 功能视角报告：本次新增/改变的能力、触发方式、可观察结果、限制和与原目标的对应关系
- 对假设、限制、延期项的说明
...
8. 输出功能视角：先说明新增/改变的能力、触发方式、可观察结果、限制和与原目标的对应关系，再说明文件修改。
...
12. **批次执行**：每完成一个批次后，在稳定点执行局部质量整理（ruff check --fix + ruff format 仅限已改 Python 文件，pyright 按项目能力执行），检查新增/修改的公共接口、关键方法和复杂逻辑是否符合注释/docstring 要求，输出功能视角，执行 spec/design/extension 漂移检查，输出批次完成状态，更新 tasks.md 记录批次进展。然后建议下一批次或停止。
...
- 覆盖多个 task 的批次必须说明统一验证计划；仅当这些 task 属于同一功能交付单元、技术闭环、真实入口或公开契约时，才可共享 validation 记录
...
- 实现内容应与已定义目标一致
- 任何额外扩 scope 都应显式指出
- 输出中必须包含功能视角，并位于文件级修改摘要之前
- 功能视角必须说明本次新增/改变的能力、用户或调用方如何触发、可观察结果、不包含什么、当前限制以及与原目标的对应关系
- 若本次无直接用户可见功能变化，必须明确写明，并说明它支撑的内部能力或间接价值
- 若实现结果与原目标存在偏差，必须先在功能视角中说明，不得只放在限制项或 Artifact 回写中
- 改动后的行为应具备验证路径
