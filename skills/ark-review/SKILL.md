...
11. 检查中文标点与尾随注释：中文 docstring / 中文注释描述句行末不得使用 `。`、`！`、`？`、`；`；业务解释类注释应写在代码上方，不写解释性尾随注释。
12. 按严重级别整理结论（见分级标准）。
...
| **Minor** | 代码清晰度问题、轻微重复、命名不清 | 函数名意图不明、局部变量命名模糊、公共接口缺少必要 docstring、L2/L3 对象只有一句空泛说明、轻度复杂度上升趋势但未失控、可后续整理的轻度重复 |
