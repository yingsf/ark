...
## 输出
- 项目概览（技术栈、布局、入口）
- 项目画像（项目类型、真实性锚点、外部依赖、契约边界、数据源元信息）
- 模块地图（各模块职责与依赖关系）
- 核心数据流与调用链
- 外部接口清单
...
## 项目画像事实
<!-- 阶段一/二写入：项目类型、真实性锚点、外部依赖、契约边界、数据源信号；数据源只记录元信息 -->
...
5. **识别项目画像初稿**
   - 推断项目类型：backend service / library SDK / CLI / frontend / data-AI / plugin / mixed / unknown
   - 识别真实性锚点：实际启动入口、包导入入口、CLI 命令、浏览器入口、宿主加载入口、核心数据处理链路等
   - 识别外部依赖信号：数据库、缓存、搜索、队列、第三方 API、文件系统、浏览器、宿主程序等
   - 识别契约边界：HTTP/MCP/API/CLI/SDK/文件格式/事件等
   - 识别数据源信号：只记录路径、文件类型、样例规模、脱敏状态、访问方式等元信息；不得读取、复制或写入敏感数据内容

6. **写入工作文档**
   - 创建 `docs/ark/.ark-analysis.md`
   - 写入「结构事实」、「技术栈事实」、「入口事实」、「项目画像事实」四个章节
...
3. **追踪数据流**
   ...
   - 对数据/AI 或文件处理项目，识别真实数据进入系统的位置、支持的文件格式、样例数据路径元信息和替身数据边界

4. **写入工作文档**
   - 将模块事实写入「模块事实」章节
   - 将数据流事实写入「数据流事实」章节
   - 更新「项目画像事实」中的真实性锚点、外部依赖、契约边界和数据源元信息
...
3. **预填充 Artifact（可选）**
   ...
   - 预填充 `docs/ark/design.md`：
     ...
     - 数据流/调用流：核心路径
     - 接口边界：外部接口清单
     - 扩展文档索引：如已存在扩展文档，仅建立摘要和可信度，不复制正文
   - 预填充内容必须标注 `<!-- 由 /ark:ark-analyze 自动生成，需人工确认 -->`
   - 如发现专题方案、契约、集成或数据源元信息需要正式成文，只建议 `/ark:ark-solution`，不得由 analyze 直接创建扩展文档
...
- 项目画像、真实性锚点、外部依赖、契约边界和数据源元信息必须区分事实、推断与不确定项
- 不得读取或复制项目数据内容；只记录数据源元信息
...
- 外部依赖：
- 契约边界：
- 数据源元信息：无 / 待确认 / <路径、格式、脱敏状态、样例范围>（不包含数据内容）
- 替身边界：
...
- 扩展文档：未创建 / 建议 `/ark:ark-solution` 创建（列出建议类型与原因）
...
- 若要编写专题方案、详细设计、接口契约、集成或数据源元信息：可显式执行 `/ark:ark-solution`
