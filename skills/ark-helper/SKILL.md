...
用户已经做出或即将做出非平凡技术选择，需要记录为什么这么选。

匹配信号：
- 提到"技术选型 / 方案对比 / 权衡 / ADR / 为什么选 A 不选 B"
- 架构、工具链、接口边界、数据模型方向发生变化
- 当前选择未来可能被质疑或需要追溯

推荐路径：
1. 若决策背景和备选方案已清楚 → `/ark:ark-decide`
2. 若选择依据还不清楚 → 先 `/ark:ark-design` 梳理备选方案，再 `/ark:ark-decide`
3. 若决策会改变任务范围 → 先 `/ark:ark-spec` 或 `/ark:ark-plan`，再 `/ark:ark-decide`

为什么：
- `/ark:ark-decide` 记录"为什么这么选"
- `design` 解释"方案怎么工作"
- `plan/tasks` 记录"接下来怎么执行"
