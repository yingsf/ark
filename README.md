...
[![Version](https://img.shields.io/badge/version-1.0.2-blue.svg)](https://github.com/yingsf/ark)
...
```
/ark-init       # 初始化 ARK 工作流
/ark-helper     # 根据场景获得流程指引
```
...
| `docs/tasks.md` | 当前有哪些任务，各自什么状态 |
| `docs/decisions.md` | 关键选择是什么，为什么这么选 |
...
| 分类 | Skill | 职责 |
|------|-------|------|
| **启动** | `/ark-init` | 初始化项目结构和工作流文件 |
| **澄清** | `/ark-intake` | 澄清任务目标、范围、规模和推荐流程 |
| | `/ark-helper` | 根据场景推荐最合适的 ARK 流程路径 |
| **分析** | `/ark-analyze` | 扫描代码库，理解架构，可选预填充 spec/design |
| **规划** | `/ark-spec` | 将需求结构化为规格文档 |
| | `/ark-design` | 编写技术设计文档 |
| | `/ark-plan` | 制定可恢复的执行计划 |
| | `/ark-tasks` | 拆分可追踪的任务列表 |
| | `/ark-decide` | 记录重要工程决策及理由 |
| **实施** | `/ark-implement` | 最小可行实现，大改动自动拆批 |
| | `/ark-debug` | 定位 bug 根因并制定低风险修复方案 |
| | `/ark-refactor` | 在保持外部行为不变的前提下改善代码结构 |
| | `/ark-review` | 审查代码变更的正确性和风险 |
| **验证** | `/ark-test` | 创建和组织测试 |
| | `/ark-validate` | 验证并记录验证证据（只验证不修复） |
| **恢复** | `/ark-handoff` | 生成交接文档，为下次会话提供恢复点 |
| | `/ark-next` | 根据当前状态裁决下一步 |
| | `/ark-sync` | 同步代码现实与 Artifact 状态 |
| **文档** | `/ark-docs` | 更新项目文档使其匹配真实实现 |
...
> 在全新 Python 项目场景下，`/ark-init` 会按推荐配置为项目接入 [uv](https://docs.astral.sh/uv/)、[ruff](https://docs.astral.sh/ruff/)、[pyright](https://github.com/RobertCraigie/pyright-python) 和 [pytest](https://docs.pytest.org/)。已有项目模式下，这些工具仅在缺失时建议安装，不会自动注入配置。
...
升级插件本身不会直接改动现有项目文件。若新版本引入了新的模板或工作流文件，建议在项目中重新运行 `/ark-init`，并按提示选择是否更新相关文件。
...
`/ark-helper` 根据你的场景推荐最合适的流程路径。它不执行任何操作，只做导航。

### 场景 1：启动新项目

```
/ark-init          → 初始化项目结构
/ark-intake        → 澄清你想做什么（需求已明确可跳过）
/ark-spec          → 将需求写成正式规格文档
/ark-design        → 编写技术设计
/ark-plan          → 制定阶段级执行计划
/ark-tasks         → 拆分为可追踪任务
/ark-implement     → 开始实现
...
### `/ark-intake` vs `/ark-spec` — "聊需求用哪个？"

| | `/ark-intake` | `/ark-spec` |
|---|---|---|
| **做什么** | 澄清任务：搞清楚你想做什么、有多大、该走什么流程 | 规格化：把澄清后的需求写成正式的规格文档 |
| **输入** | 原始用户请求（可能模糊） | 已澄清的需求、领域上下文、验收期望 |
| **输出** | 任务理解、规模判断、约束、假设、推荐流程路径 | `docs/spec.md`：目标、范围、非目标、验收标准 |
...
```
"我想加个用户认证"  →  /ark-intake（澄清范围）  →  /ark-spec（写规格）
"需求我清楚了"      →  直接 /ark-spec（跳过 intake）
"这个太慢了"        →  /ark-intake（High fuzziness，帮你表达需求）
```

### `/ark-plan` vs `/ark-tasks` — "规划工作用哪个？"

| | `/ark-plan` | `/ark-tasks` |
|---|---|---|
| **做什么** | 制定阶段级执行策略 | 拆分为可追踪的粒度任务 |
| **产出** | `docs/plan.md`：阶段划分、里程碑、风险、验证策略 | `docs/tasks.md`：任务列表 + 状态（Todo/Doing/Done/Blocked） |
| **粒度** | 阶段级——"先做 A，再做 B，B 完成后验证" | 任务级——"任务 1：创建用户模型（Todo，高优先级）" |
| **回答的问题** | "分几个阶段？每个阶段的入口/出口是什么？" | "当前有哪些具体任务？各自什么状态？" |

**关系**：`plan` 先出阶段策略，`tasks` 再把阶段拆成可追踪任务。`plan` 的建议下一步就是 `/ark-tasks`。

```
/ark-plan  →  "分 3 个阶段：数据层 → API 层 → 集成测试"
/ark-tasks  →  "阶段 1 拆为：T1 创建模型(Doing) T2 写迁移(Todo) T3 补测试(Todo)"
```

### `/ark-test` vs `/ark-validate` — "验证代码用哪个？"

| | `/ark-test` | `/ark-validate` |
|---|---|---|
| **做什么** | 写测试代码并执行 | 记录"验证了什么、证据是什么" |
| **改代码吗** | **是** — 创建/修改测试文件 | **绝不** — 纯记录，连一行 import 都不改 |
| **产出** | 测试文件 + 执行结果摘要 | `docs/validation.md`：已验证项、未覆盖项、风险结论 |
| **回答的问题** | "测试怎么写？跑出来的结果是什么？" | "当前验证了什么？还有什么没验证？验证强度够不够？" |

**关系**：`test` 产生测试代码和执行结果，`validate` 把这些结果（连同其他验证手段）记录为正式证据。`test` 的输出是 `validate` 的输入。

```
/ark-test     →  写了 5 个测试，跑出 4 passed 1 failed
/ark-validate  →  记录：已验证 X（测试通过），未验证 Y（需要真实环境），风险 Z
```

### `/ark-analyze` vs `/ark-review` — "看代码用哪个？"

| | `/ark-analyze` | `/ark-review` |
|---|---|---|
...
```
接手陌生项目  →  /ark-analyze（建立全局认知）
实现完一个功能  →  /ark-review（检查改动质量）
```

### `/ark-sync` vs `/ark-next` — "中断后恢复用哪个？"
...
**关系**：如果你怀疑文档过期了，先 `/ark-sync` 修状态，再 `/ark-next` 看该做什么。如果文档状态可信，直接 `/ark-next` 即可。

```
隔了 3 天回来，不确定文档是否过期  →  /ark-sync → /ark-next
刚中断 10 分钟，文档应该还准    →  直接 /ark-next
```
...
### 3. 批次实施机制 (`/ark-implement`)

改动涉及 3+ 文件时自动触发批次拆分。每个批次定义 4 要素：目标、涉及文件、修改锚点、完成信号。批次完成点就是天然的中断安全点——所有文件修改已落盘、局部检查已执行。即使会话中断，下次也能从上一个完成批次继续。

### 4. 只验证不修复 (`/ark-validate`)

验证和修复是两件不同的事。`/ark-validate` 的硬边界是：**只记录验证状态，绝不修改代码**。即使发现只差一行 import，也必须交给 `/ark-debug` 处理。mock 环境的验证结果永远不报告为"通过"。

### 5. 裁决优先级序列 (`/ark-next`)

中断恢复不是拍脑袋。`/ark-next` 按确定性决策链裁决：状态可信性 → 验证闭环 → 活跃执行项 → 回退规划层。依次读取 handoff → tasks → plan → validation → spec/design，找到第一个未闭合的环节。

### 6. Inspect & Respect (`/ark-init` Mode B)
...
不受上下文窗口限制。`/ark-handoff` 记录完成/未完成/风险/关键文件，`/ark-next` 读取所有 Artifact 裁决恢复路径。即使隔了三天、换了设备，只要文件在，状态就在。

### 10. 预填充与审查确认分离

`/ark-analyze` 可以从代码反推并预填充 `spec.md` 和 `design.md`，但会标注为 `<!-- 由 /ark-analyze 自动生成，需人工确认 -->`。后续必须经过 `/ark-spec` 或 `/ark-design` 审查确认，不能直接当作已确认的需求或设计使用。

---

## 项目结构

安装 ARK 插件并执行 `/ark-init` 后，你的项目会获得以下结构（以全新项目为例）：

```
my_project/
├── docs/                        # ARK Artifact 目录
│   ├── spec.md                  # 需求规格
│   ├── design.md                # 技术设计
│   ├── plan.md                  # 执行计划
│   ├── tasks.md                 # 任务列表
...
可以。`/ark-init` 的模式 B 专为已有项目设计，只注入工作流文件，不修改任何已有代码和配置。
...
初始生成的 Artifact 文件保持轻量，均为纯文本，适合直接纳入 Git 管理并随项目演进持续维护。`/ark-sync` 可以检测并清理过期内容。
