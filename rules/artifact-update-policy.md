...
- **验证可聚合但不能泛化**：多个 task 可共享一条 validation 记录，但必须同属一个功能闭环、同一 batch、同一真实入口或同一公开契约，并在记录中写明覆盖范围
...
`tasks.md` 的任务默认应是功能交付单元或可验证技术闭环。文件、函数、配置、测试断言等低层步骤应写入「实施要点」，不默认拆成独立 task。多个 Done 任务可以引用同一条 `validation.md` 记录，但该记录必须列出覆盖任务、覆盖原因和未覆盖任务。
...
一条验证记录可以覆盖多个任务，但仅限同一功能闭环、同一 batch、同一真实入口或同一公开契约。记录中必须写明覆盖任务、覆盖原因、未覆盖任务和不覆盖原因；不得用宽泛验证记录覆盖无关任务。
