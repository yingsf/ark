...
`tasks.md` 默认按功能交付单元或可验证技术闭环拆分，不按文件、函数、类、配置项或测试断言拆分。低层步骤写入任务的实施要点。

上游约束：`spec` 的验收标准不写成实现步骤；`design` 给出技术闭环建议，不生成执行清单；`plan` 给出建议 task 边界和不建议拆分项。

当前执行窗口建议 3-8 个 task。多个强相关 task 可以共享一条 validation 记录，但必须同属一个功能闭环、同一 batch、同一真实入口或同一公开契约，并明确覆盖范围。
